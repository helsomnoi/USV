...
legendSize = 6  # legend size
figSize1 = [23, 15]  # figure1 size in cm
figSize2 = [23, 15]  # figure2 size in cm
...
def plotVehicleStates(simTime, simData, figNo, psi_d):
    # Time vector
    t = simTime

    # State vectors (в плоском движении интересны только x, y, psi, u, v и r)
    x = simData[:, 0]
    y = simData[:, 1]
    psi = R2D(ssa(simData[:, 5]))
    u = simData[:, 6]
    v = simData[:, 7]
    r = R2D(simData[:, 11])  # yaw rate
    
    u_control = R2D(simData[:, 12])  # control input, commands
    u_actual = R2D(simData[:, 13])  # actual control input
    #motors_speed = R2D(simData[:, 14])

    # Speed (нормальная скорость в плоскости)
    U = np.sqrt(u ** 2 + v ** 2)

    # Course angle (угол курса, chi = psi + crab angle)
    chi = R2D(ssa(simData[:, 5] + np.arctan2(v, u)))

    # Plots
    plt.figure(figNo, figsize=(cm2inch(figSize1[0]), cm2inch(figSize1[1])), dpi=dpiValue)

    # 1. Положение на плоскости (x, y)
    plt.subplot(2, 2, 1)
    plt.plot(y, x)
    plt.legend(["North-East с.к., м"], fontsize=legendSize)
    plt.grid()

    # 2. управление
    plt.subplot(2, 2, 2)
    plt.plot(t[:len(t)-1], u_control,t[:len(t)-1],u_actual)
    plt.legend(["u_command", "u_actual"],fontsize=legendSize,)
    plt.grid()
    

    # 3. Углы курса и рысканья
    plt.subplot(2, 2, 3)
    plt.plot(t[:len(t)-1], psi)
    plt.axhline(psi_d, color='r', linestyle='--', label="Заданный угол, град")
    plt.legend(["Курсовой угол, град.", "Заданный угол, град"], fontsize=legendSize) #"Course angle (deg)"], 
    plt.xlabel("Время, сек", fontsize=10)
    plt.grid()

    # 4. Скорости в плоскости
    plt.subplot(2, 2, 4)
    plt.plot(t[:len(t)-1], u,t[:len(t)-1], r) # t, v, 
    plt.legend(
        ["Линейная скорость, м/с", "Угловая скорость, град/сек"],
        fontsize=legendSize,
    )
    plt.xlabel("Время, сек", fontsize=10)
    plt.grid()
...
"""def plotVehicleStates(simTime, simData, figNo):
    ...
    plt.title("Vehicle states", fontsize=10)
...
"""def plotControls(simTime, simData, vehicle, figNo):
    ...
    for i in range(0, vehicle.dimU):
        ...
        plt.xlabel("Время, сек", fontsize=8)
        plt.grid()"""
...
def plot3D(simData,numDataPoints,FPS,filename,figNo):
    ...
    def anim_function(num, dataSet, line):
        ...
        ax.view_init(elev=35.0, azim=-120.0)
    ...
    ax.set_xlabel('X / East, м')
    ax.set_ylabel('Y / North, м')
    ax.set_zlim3d([-10, 10])                   # default depth = -100 m
    
    if np.amax(z) > 100.0:
        ax.set_zlim3d([-np.amax(z), 20])
        
    ax.set_zlabel('-Z / Down, м')
